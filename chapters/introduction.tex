...
As noted in §\ref{introintro}, there is a reasonably large existing literature {focusing} on specific aspects of \ili{Arabic} and contact-induced change. For reviews of much of this literature, readers are referred to the relevant chapters of the present volume. Here we simply present the key works for ease of reference in the following (non-comprehensive) thematic bibliography.
...
Despite these descriptive gaps, the twenty-nine chapters included in this volume collectively have the merit of discussing a wide range of contact situations involving \ili{Arabic} (balanced {bilingualism}, unbalanced {bilingualism}, pidginization and creolization), which cover a broad geographical area and lengthy timespan, thus giving a close-to-comprehensive picture of the currently known facts of \ili{Arabic} and contact-induced change.
...
As an example of the borrowing of {derivational} morphology, Benkato (this volume)\ia{Benkato, Adam@Benkato, Adam} cites the \ili{Moroccan} \ili{Arabic} circumfix \textit{tā-...-t}, borrowed from \ili{Berber}, as the regular means of deriving nouns of professions and traits, as in \textit{tānǝžžā{\R}t} ‘carpentry’ (< \textit{nǝžžā\R} `carpenter').


Finally, in the domain of verbal {inflection}, we can point to the {contact-induced grammaticalization} in \ili{NENA} of a propspective {future} marker \textit{zi-}, as in \REF{introgoing}, on the model of \ili{Arabic} \textit{raḥ-} with the same function, both deriving from elements with the basic meaning of `going'.
...
In the domain of morphosyntax, van Putten (this volume)\ia{van Putten, Marijn@van Putten, Marijn} cites Wilmsen's (\citeyear{Wilmsen2010}) example of {imposition} in the treatment of direct and indirect pronominal objects in MSA. As Wilmsen shows, native speakers of \ili{Egyptian} \ili{Arabic} writing MSA tend to impose their native system, such that the direct object cliticizes to the verb, as in \REF{introwilmsen1}, whereas native speakers of \ili{Lebanese} \ili{Arabic} tend to impose their native system, such that it is the indirect object that cliticizes to the verb, as in \REF{introwilmsen2}.
