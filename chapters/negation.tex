...
Matras (\citeyear{Matras2012}: 350–351) describes two syntactic contexts in which negators borrowed from Palestinian Arabic are the only options in this variety of Domari. The first is with Arabic-derived modal auxiliaries that take Arabic suffix inflection, as in \textit{bidd-} ‘want’ in \REF{dom}. Here negation is typically with the Palestinian Arabic stage-III construction \textit{{}-š} (without \textit{mā}), as it is would be also in Palestinian Arabic.
